...
// Analytic (closed-form) Jacobians for the TEB edges are the default. Define
// USE_ANALYTIC_JACOBI=0 (or pass -DTEB_USE_ANALYTIC_JACOBI=OFF to colcon build)
// to fall back to g2o's numeric differentiation, which re-evaluates
// computeError() once per vertex DOF per iteration.
#ifndef USE_ANALYTIC_JACOBI
#define USE_ANALYTIC_JACOBI 1
#endif
...
public:
  ...
  */     
  ...
  // The no-arg linearizeOplus() override hides the base's workspace overload;
  // re-expose it so callers can bind a JacobianWorkspace before linearizing.
  using g2o::BaseUnaryEdge<D, E, VertexXi>::linearizeOplus;
...
public:
  ...
  */     
  ...
  // The no-arg linearizeOplus() override hides the base's workspace overload;
  // re-expose it so callers can bind a JacobianWorkspace before linearizing.
  using g2o::BaseBinaryEdge<D, E, VertexXi, VertexXj>::linearizeOplus;
...
public:
  ...
  */     
  ...
  // The no-arg linearizeOplus() override hides the base's workspace overload;
  // re-expose it so callers can bind a JacobianWorkspace before linearizing.
  using g2o::BaseMultiEdge<D, E>::linearizeOplus;
